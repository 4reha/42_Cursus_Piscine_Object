...
void NeedCourseCreationForm::execute()
{
	...
	CourseList::getInstance()->add(new Course("Course_" + std::to_string(size)));

	Professor *prof = dynamic_cast<Professor *>(this->_demandant);
	Course *course = CourseList::getInstance()->getList().back();
	if (prof)
	{
		course->assign(prof);
...
void SubscriptionToCourseForm::execute()
{
	...
	if (student)
	{
		...
		{
			Course *course = CourseList::getInstance()->getList()[i];
