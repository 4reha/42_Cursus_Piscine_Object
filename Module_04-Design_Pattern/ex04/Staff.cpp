...
// void Professor::update(Event _event)
// {
// 	if (_event == RingBell)
// 	{
...
void Professor::doClass()
{
	...
	Classroom *classroom = RoomList::getFreeClassroom();
	if (classroom)
	{
		this->_currentRoom = classroom;
		...
		for (int i = 0; i < _currentCourse->getStudents().size(); i++)
		{
			if (_currentCourse->getStudents()[i]->getFinishedCourse() == _currentCourse)
			{
				if (headmaster)
					headmaster->notify(_currentCourse->getStudents()[i], CourseFinished);
			}
		}
	}
}
